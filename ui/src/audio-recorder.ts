...
class AudioRecorder {
    ...
    private async start(): Promise<void> {
        ...
        if (!mime) {
            await warnDialog("Browser doesn't support audio recording.");
            return;
        }
        this.mimeType = mime;
    ...
    private stop(): void {
        ...
        try {
            ...
            // stop() threw → the 'onstop' handler won't fire → finalize() never
            // runs, leaving the UI stuck on "Saving…" with the button disabled.
            // Reset out of that dead end and tell the user.
            console.warn('[recorder] stop failed:', e);
            this.state = 'idle';
            this.updateUi();
            removeTask('recording');
            void warnDialog(`Could not stop the recording cleanly:\n${e}`);
        }
    }
    ...
    private async finalize(ctx: AudioContext): Promise<void> {
        ...
        try {
            ...
            await warnDialog(`Could not finalise recording:\n${e}`);
        } finally {
            this.state = 'idle';
            this.updateUi();
            removeTask('recording');
    ...
    private async promptAndSave(bytes: Uint8Array, seconds: number): Promise<void> {
        ...
        try {
            ...
            await warnDialog(`Could not save:\n${e}`);
        }
    }
