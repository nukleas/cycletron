...
//! and the pre-request refresh in `send_message` dispatch through here rather
//! than string-matching `"grok"` / `"codex"` independently, so those callers
//! never name a provider and a new OAuth backend is one arm in each match below.
