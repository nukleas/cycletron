...
fn resolve_patterns(
        ...
        && !file.is_empty()
    {
        ...
        for binding in &file.bindings {
            // NOTE: engine's `Expr::shift_string_offsets` was removed upstream;
            // binding parse-error offsets are now relative to the binding text.
...
fn find_local_samples_manifest() -> Option<PathBuf> {
    if let Ok(env) = std::env::var("ROBOSTRUDEL_SAMPLES") {
...
mod tests {
    ...
    fn tmp(prefix: &str, ext: &str) -> PathBuf {
        let stamp = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("robostrudel-{prefix}-{stamp}.{ext}"))
    }
