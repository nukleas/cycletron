...
signature: soft boom-bap, upright bass, kalimba line
---
...
Folktronica — soft boom-bap, upright bass, kalimba line. Tempo 105–115 BPM in g major. This skeleton is composed straight from the genre spec (aligned drum grid, in-key bass, diatonic chords, a generated lead), round-trip verified so it always parses and plays. Use it as a seed: lift the parts you want, then layer and edit.

## Full skeleton

```strudel
// folktronica in G major (generated) — soft boom-bap, upright bass, kalimba line
setbpm(110);

stack(
  s("bd ~ ~ ~ ~ ~ bd ~ ~ ~ bd ~ ~ ~ ~ ~, ~ ~ ~ ~ sd ~ ~ ~ ~ ~ ~ ~ sd ~ ~ ~").gain(0.71).every(4, x => x.fast(2)),
  note("g2 ~ d3 ~ g2 ~ f#3 ~").s("gm_acoustic_bass").gain(0.49),
  note("<[g3, b3, d4, f#4] [d4, f#4, a4, c5] [e4, g4, b4, d5] [c4, e4, g4, b4]>").s("wt_piano").release(0.5).gain(0.34).room(0.4).pan(0.42),
  note("<[d5 ~ g5 ~ c6 ~ b5 ~] [d5 ~ g5 ~ c6 ~ b5 ~] [f#5 ~ b5 ~ e6 ~ d6 ~] [b5 ~ b5 ~ b5 ~ f#5 ~]>").s("gm_kalimba").room(0.4).delay(0.25).gain(0.28).pan(0.62)
)
```
