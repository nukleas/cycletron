...
stack(
  s("bd ~ ~ ~ ~ ~ ~ ~ ~ ~ bd ~ ~ ~ ~ ~, ~ ~ ~ ~ sd ~ ~ sd ~ ~ ~ ~ sd ~ ~ ~, ~ hh ~ hh ~ hh ~ hh ~ hh ~ hh ~ hh ~ hh").gain(0.73).every(4, x => x.fast(2)),
  note("a1 ~ ~ g2 ~ ~ a2 ~").s("supersaw").lpf(600).resonance(10).gain(0.4),
  note("<[a3, c4, e4, g4] [f4, a4, c5, e5] [g4, b4, d5, f5] [f4, a4, c5, e5]>").s("sawtooth").struct("~ 1 ~ 1").release(0.14).lpf(1700).gain(0.26).room(0.3).pan(0.42)
)
```
