...
//!   cargo run -p robostrudel-gen --example sectionize_song -- file.strudel [L]

use robostrudel_gen::{factor, song};
use robostrudel_gen::mini::Mini;
use robostrudel_gen::verify::validate_doc;
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: sectionize_song <file> [L]");
    let l: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(8);
    let src = std::fs::read_to_string(&path).expect("read");
    let wraps = ["s(\"{}\").gain(0.38)", "note(\"{}\").sound(\"sine\")", "note(\"{}\").sound(\"supersquare\")"];
    ...
    for (t, body) in backtick_bodies(&src).into_iter().enumerate() {
        ...
        println!("track {}: {} bars → {} sections of {l}, {} unique  |  body {} → doc {} chars ({}% smaller)  [plays: {}]",
            t+1, bars.len(), n_sections, unique, orig_body, new_doc,
            if orig_body>0 {100*orig_body.saturating_sub(new_doc)/orig_body} else {0}, plays);
        if t == 0 { println!("--- drum track, sectionized ---\n{doc}"); }
    }
}
