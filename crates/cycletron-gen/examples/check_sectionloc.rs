...
fn main() {
    ...
    let sec: Vec<_> = haps
        .iter()
        .filter_map(|h| match h.context.get(&ContextKey::SectionLoc) {
            Some(Value::Location(l)) => Some((l.start(), l.end())),
            _ => None,
        })
        .collect();
    let note = haps
        .iter()
        .filter(|h| h.context.get(&ContextKey::Locations).is_some())
        .count();
    ...
    assert!(
        sec.iter().any(|(s, _)| *s == g_off),
        "section loc must point at `g`"
    );
