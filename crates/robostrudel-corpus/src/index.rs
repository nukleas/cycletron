...
impl CorpusIndex for InMemoryCorpusIndex {
    ...
    fn get_source(&self, id: &str) -> robostrudel_core::Result<String> {
        let entry = self
            .get(id)
            .ok_or_else(|| robostrudel_core::Error::Corpus(format!("entry not found: {id}")))?;
        ...
        crate::loader::load_source(&self.corpus_path, &entry.filename)
            .map_err(|e| robostrudel_core::Error::Corpus(e.to_string()))
    }
