...
///  - `dull`: an near-empty top end (no air/definition) — a gap the symbolic
///    model missed until the Gemini listener kept catching "no hats, no top end".
...
mod tests {
    ...
    fn dark_mix_with_no_top_end_is_flagged_dull() {
        ...
        // (The gap the Gemini listener kept catching; now symbolic.)
        let doc = r#"stack(
          s("bd*4").gain(0.8),
